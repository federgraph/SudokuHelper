...
<history>
 ...
 Last modified by PB 2021-11-09<p>
...
type
  {!
   This class just serves as a container for static methods,
  ...
  public
    ...
    class procedure SaveToFile(const aSudoku: ISudokuHelper; const aFilename: string); static;
    class procedure SaveToStream(const aSudoku: ISudokuHelper; aStream: TStream); static;
  end;
...
class function TSudokuFiler.LoadFromFile(const aFilename: string): ISudokuHelper;
const
...
<returns>The interface for the created helper holding the Sudoku.</returns>
<exception cref="EParameterCannotBeNil">is raised if aStream is nil.</exception>
<remarks>
 ...
 Reading starts at the current stream position!
</remarks>
}
...
{!
<summary>Save a Sudoku to a file. </summary>
...
<exception cref="EParameterCannotBeNil">is raised if aSudoku is nil.</exception>
<remarks>
 The method will overwrite an existing file with the passed name. If
 the target folder named in the path does not exist it will be created.
</remarks>
}
class procedure TSudokuFiler.SaveToFile(const aSudoku: ISudokuHelper; const aFilename: string);
const
...
begin
  ...
  //LPath := ExtractFileDir(aFileName); // GetDirectoryName(aFilename);
...
class procedure TSudokuFiler.SaveToStream(const aSudoku: ISudokuHelper; aStream: TStream);
const
  CBufSize = 4096 * 4;

...
type
  ...
  { The Sudoku blocks form a 2D array,
    ...
    for a 12x12 Sudoku a 3x4 array,
    for a 16x16 Sudoku a 4x4 array.
    The block definitions are static,
    and using dynamic arrays we can handle them in the base datastorage class.
    All the descendants have to supply the block width and height.
  }
  TSudokuBlockEntry = record
    Col: TSudokuCellIndex;
    Row: TSudokuCellIndex;
...
constructor TBaseSudokuDatastorage.Create(
  const AMaxValue, ABlockHeight, ABlockWidth: TSudokuValue;
...
function TBaseSudokuDatastorage.GetCell(aCol, aRow: TSudokuCellIndex): ISudokuCell;
const
  CProcName = '.GetCell';
begin
...
function TBaseSudokuDatastorage.GetCellInBlockLocation(aCol, aRow: TSudokuCellIndex): TCellInBlockLocation;
var
...
function TBaseSudokuDatastorage.IsValidBlockValue(aCol, aRow: TSudokuCellIndex; aValue: TSudokuValue): Boolean;
var
  LBlock: TSudokuBlock;
  LEntry: TSudokuBlockEntry;
  I, K: Integer;
...
function TBaseSudokuDatastorage.IsValidColValue(aCol: TSudokuCellIndex; aValue: TSudokuValue): Boolean;
var
  LCell: TSudokuCell;
...
function TBaseSudokuDatastorage.IsValidRowValue(aRow: TSudokuCellIndex; aValue: TSudokuValue): Boolean;
var
  LCell: TSudokuCell;
...
function TBaseSudokuDatastorage.IsValueValid(aCol, aRow: TSudokuCellIndex; aValue: TSudokuValue): Boolean;
var
  LCell: TSudokuCell;
begin
...
var
  ...
  begin
    if aCell.IsEmpty and (LValue in aCell.Candidates) then
    begin
...
procedure TBaseSudokuDatastorage.ValidateCellCoord(aCol, aRow: TSudokuCellIndex; const aProcName: string);
const
  CColumn = 'Column';
  CRow = 'Row';
var
...
function TBaseSudokuDatastorage.TSudokuStack.MarkExists(const aName: string): Boolean;
var
  LList: TStringlist;
begin
...
constructor TBaseSudokuDatastorage.TStackMark.Create(const aCount: Integer; const aName: string);
begin
  Count := aCount;
  Name := aName;
end;
